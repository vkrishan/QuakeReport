...
public final class QueryUtils {
    ...
    public static ArrayList<EarthquakeDataModel> extractEarthquakes() {
        ...
        try {
            ...
            // ROOT -> FEATURES [] -> PROPERTIES objext -> mag
            // ROOT -> FEATURES [] -> PROPERTIES objext -> place
            // ROOT -> FEATURES [] -> PROPERTIES objext -> time

            // TODO: Parse the response given by the SAMPLE_JSON_RESPONSE string and
            // build up a list of Earthquake objects with the corresponding data.
            JSONObject baseEarthquakeResponse = new JSONObject(SAMPLE_JSON_RESPONSE);

            JSONArray featuresArr = baseEarthquakeResponse.optJSONArray("features");

            // loop through the array
            for(int i=0; i<featuresArr.length(); i++){

                JSONObject temp = featuresArr.getJSONObject(i); // store object in array
                ...
                EarthquakeDataModel earthquake = new EarthquakeDataModel(magnitude, place, time);
